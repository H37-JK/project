from uuid import UUID

from fastapi import APIRouter, Depends, HTTPException
from sqlmodel import select
from typing import Annotated
from backend.model.user import User
from backend.passlib.jwt_token import get_current_user
from backend.db.engine import SessionDep
from backend.logs.logging_route import LoggingRoute
from backend.model.api.api_request_history import ApiRequestHistory

router = APIRouter (
    prefix = "/api-request-history",
    tags = ["api-request-history"],
    responses = {404: {"description": "Nou Found"}},
    route_class = LoggingRoute,
)

@router.get("/get/api-requests-histories")
async def get_api_request_histories (
    session: SessionDep,
    current_user: Annotated[User, Depends(get_current_user)]
):
    return session.exec(select(ApiRequestHistory).where(ApiRequestHistory.user_id == current_user.id)).all()


@router.get("/get/api-request-history/{id}")
async def get_api_request_history (
    session: SessionDep,
    current_user: Annotated[User, Depends(get_current_user)],
    id: int
):
    return session.exec(select(ApiRequestHistory).where(ApiRequestHistory.id == id, ApiRequestHistory.user_id == current_user.id)).one_or_none()


@router.post("/create/api-request-history/{api_request_id}")
async def create_api_request_history (
    session: SessionDep,
    current_user: Annotated[User, Depends(get_current_user)],
    api_request_history: ApiRequestHistory,
    api_request_id: UUID
) -> ApiRequestHistory:
    api_request_history.user_id = current_user.id
    api_request_history.api_request_id = api_request_id
    session.add(api_request_history)
    session.commit()
    session.refresh(api_request_history)
    return api_request_history


@router.patch("/update/api-request-history/{id}")
async def update_api_collection (
    session: SessionDep,
    current_user: Annotated[User, Depends(get_current_user)],
    request_api_request_history: ApiRequestHistory,
    id: UUID
) -> ApiRequestHistory:
    api_request_history = session.exec(select(ApiRequestHistory).where(ApiRequestHistory.user_id == current_user.id, ApiRequestHistory.id == id)).one_or_noe()
    if not api_request_history:
        raise HTTPException(status_code = 404, detail = "Request not found")

    update_data = request_api_request_history.model_dump(exclude_unset = True)
    api_request_history.sqlmodel_update(update_data)

    session.add(api_request_history)
    session.commit()
    session.refresh(api_request_history)

    return api_request_history


@router.delete("/delete/api-request-history/{id}")
async def delete_api_collection (
    session: SessionDep,
    current_user: Annotated[User, Depends(get_current_user)],
    id: UUID
):
    api_request_history = session.exec(select(ApiRequestHistory).where(ApiRequestHistory.user_id == current_user.id, ApiRequestHistory.id == id)).one_or_noe()
    if not api_request_history:
        raise HTTPException(status_code = 404, detail = "Request not found")

    session.delete(api_request_history)
    session.commit()

    return None